...
int CoupOpti(posGraph position, int parentID, int moveID, int isJoueurAuTrait, int coupopti[], int evaluation[]) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (positionConvert[i] == '.') {
                ...
                if (parentID == -1) {
                    ...
                    val = max(val, -CoupOpti(newPosition, 0, nombreDeNoeudUlt, 1 - isJoueurAuTrait, coupopti, evaluation));
                    coupopti[i] = i;
                    evaluation[i] = val;
                } else {
                    val = max(val, -CoupOpti(newPosition, moveID, nombreDeNoeudUlt, 1 - isJoueurAuTrait, coupopti, evaluation));
                }
            }
